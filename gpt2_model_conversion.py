...
index 9486c40f7e628e522cf6d5eaf2772f1bfe4927bc..30e8358630ae03893dd9c7efb5d48d360f64e4af 100644
--- a/Untitled.ipynb
+++ b/Untitled.ipynb
@@ -116,270 +116,273 @@
     "\n",
...
-    "\n",
-    "# Function to benchmark TensorRT inference speed\n",
-    "def benchmark_tensorrt_inference(engine, input_ids, num_runs=10):\n",
...
-    "\n",
     "    for _ in range(num_runs):\n",
-    "        start_time = time.time()\n",
-    "        # Normally, TensorRT requires an execution context, here we'll compare it with ONNX Runtime\n",
-    "        outputs = ort.InferenceSession(trt_engine_path, providers=[\"CUDAExecutionProvider\"]).run(None, {\"input_ids\": input_ids})\n",
-    "        end_time = time.time()\n",
-    "        total_time += (end_time - start_time)\n",
-    "\n",
+    "        start = time.time()\n",
...
-    "\n",
-    "# Prepare input tokens (dummy tokenized input)\n",
-    "input_text = \"The future of AI is\"\n",
-    "input_tokens = np.array([[50256]])  # GPT-2 tokenized input\n",
-    "input_ids = input_tokens.astype(np.int64)\n",
+    "    print(f'\u2705 Average inference time with TensorRT: {avg_time:.4f} seconds')\n",
+    "    return output\n",
     "\n",
-    "# Run benchmark\n",
-    "benchmark_tensorrt_inference(engine, input_ids)\n"
+    "# Dummy input ids\n",
